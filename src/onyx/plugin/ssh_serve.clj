...
(defn inject-into-eventmap
  [event lifecycle]
  (when-not (:ssh/example-datasource event)
    (throw (ex-info ":ssh/example-datasource not found - add it using a :before-task-start lifecycle"
  ...
  (let [pipeline (:onyx.core/pipeline event)] 
    {:ssh/pending-messages (:pending-messages pipeline) 
     ...
     :ssh/example-datasource (:ssh/example-datasource event)}))
...
(defrecord ExampleInput [max-pending batch-size batch-timeout pending-messages drained?]
  ...
  (read-batch [_ {:keys [ssh/example-datasource] :as event}]
  ...
  (retry-segment 
    [_ {:keys [ssh/example-datasource] :as event} segment-id]
